...
export default function ReplaceableCouplesTransition(env: Environment): State {
  ...
  const currentMaleDirection = currentState.male && Direction[currentState.male.direction as TDirection]
  ...
  if (currentState.nextMale) {
    if (env.isAccessible(...currentState.nextMale)) {
      ...
      if (maleLocation.male) {
        ...
        if (
          checkCoordinatesZero(currentState.nextMale, maleDirection.coordinates as [number, number])
            && (
                !maleLocation.female ||
            (currentState.female && currentMaleDirection && checkCoordinatesZero(maleLocation.nextMale as [number, number], currentMaleDirection.coordinates as [number, number]))
            )
        ) {
          console.log(
            'moving male ',
  ...
  if (currentState.female || !currentState.male) {
    for (let cor of Object.values(Direction).map(d => d.coordinates)) {
      ...
      if (
          (!state.female && checkCoordinatesZero(
          cor as [number, number],
          maleDirection.coordinates as [number, number],
        ) && (!currentState.female || currentState.matingScore > calcMatingScore(currentState.female, state.male))) ||
        (state.female && currentState.female &&
          currentState.matingScore + state.matingScore >
            calcMatingScore(currentState.female, state.male) + (currentState.male ? calcMatingScore(state.female, currentState.male) : 100))
      ) {
        console.log(
    ...
    if (env.isAccessible(...currentMaleDirection.coordinates)) {
      ...
      if (
        ...
        if (nextLocation.male) {
          console.log('moving current male ', currentState.male.character, ' got instead ', nextLocation.male.character)
          nextState.male = {
        ...
        if (currentState.male.moveAttempts && currentState.male.moveAttempts > 1) {
          ;(nextState.male as Person).direction = randomDirection()
            console.log('move failed. changing direction ', currentState.male.character, ' was ', currentState.male.direction, ' now ', (nextState.male as Person).direction )
        } else {
      ...
      ;(nextState.male as Person).direction = randomDirection()
        console.log('unaccaccible direction. changing direction ', currentState.male.character, ' was ', currentState.male.direction, ' now ', (nextState.male as Person).direction )
    }
    return nextState
  }
